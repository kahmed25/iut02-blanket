...
def fetch_excel_from_url(url: str, cache_dir: Path, cache_expiration: int = 300) -> BytesIO:
    """
    Fetch Excel file from URL with caching
    
    Args:
        url: URL to fetch Excel file from
    ...
    if (_excel_cache["data"] is not None and 
        _excel_cache["url"] == url and 
        (current_time - _excel_cache["timestamp"]) < cache_expiration):
        # Return cached data
        _excel_cache["data"].seek(0)  # Reset file pointer
    ...
    try:
        ...
        # Update cache
        _excel_cache["data"] = excel_data
        _excel_cache["timestamp"] = current_time
        _excel_cache["url"] = url
        
        # Also save to disk cache
        url_hash = hashlib.md5(url.encode()).hexdigest()
        cache_file = cache_dir / f"excel_{url_hash}.xlsx"
        cache_file.write_bytes(response.content)
        
        excel_data.seek(0)  # Reset file pointer
        return excel_data
        
    except requests.RequestException as e:
        # If fetch fails, try to use disk cache
        url_hash = hashlib.md5(url.encode()).hexdigest()
        cache_file = cache_dir / f"excel_{url_hash}.xlsx"
        ...
        raise Exception(f"Failed to fetch Excel from URL: {str(e)}")


def clear_excel_cache():
    """
    Clear the in-memory Excel cache
    """
    global _excel_cache
    _excel_cache = {
        "data": None,
...
def parse_excel(file_path: Union[Path, BytesIO, str], is_url: bool = False) -> Dict[str, Any]:
    """
    Parse Excel file and return structured JSON data
    
    Args:
        file_path: Path to the Excel file, BytesIO object, or file path string
        is_url: Whether the file was fetched from a URL
    ...
    try:
        ...
        if isinstance(file_path, Path):
            ...
            file_name = "remote_excel_file.xlsx"
        else:
